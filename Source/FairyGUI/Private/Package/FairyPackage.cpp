...
void UFairyPackage::Load(FairyGUI::FByteBuffer* Buffer)
{
	...
	Buffer->Seek(indexTablePos, (int32)FairyGUI::EPackageSegmentIndex::StringTable);
	...
	Buffer->Seek(indexTablePos, (int32)FairyGUI::EPackageSegmentIndex::DependencyAndBranch);
	...
	Buffer->Seek(indexTablePos, (int32)FairyGUI::EPackageSegmentIndex::CompontentList);
	FString assertPath = FPaths::GetPath(AssetPath.ToString());
	FString assertFileName = FPaths::GetBaseFilename(AssetPath.ToString());
	...
	for (int32 i = 0; i < itemNum; i++)
	{
		...
		{
			FString file = assertFileName + "_" + FPaths::GetBaseFilename(pi->File.ToString());
			pi->File = FName(assertPath + "/" + file + "." + file);
			break;
		}
		...
		{
			pi->File = FName(assertPath + pi->File.ToString());
			break;
		}
	...
	Buffer->Seek(indexTablePos, (int32)FairyGUI::EPackageSegmentIndex::Atlas);
	...
	for (int32 i = 0; i < atlasNum; i++)
	{
		...
		FName atlasId = Buffer->ReadFNameFromCache();
		FName PackageItemId = Buffer->ReadFNameFromCache();
		const TSharedPtr<FFairyPackageItem>& atlasPackageItem = ItemsByID[PackageItemId];
		...
		Sprites.Add(atlasId, sprite);

		Buffer->SetPos(nextPos);
	}

	//if (Buffer->seek(indexTablePos, (int32)FairyGUI::EPackageSegmentIndex::PixelHitTest))
	//{
