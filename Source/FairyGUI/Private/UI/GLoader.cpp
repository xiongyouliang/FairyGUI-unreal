...
void UGLoader::LoadFromPackage(const FString& ItemURL)
{
	...
	if (ContentItem.IsValid())
	{
		...
		if (ContentItem->GetPackageItemType() == EPackageItemType::Image)
		{
		...
		else if (ContentItem->GetPackageItemType() == EPackageItemType::MovieClip)
		{
			Content->SetClipData(ContentItem->MovieClipData);
			UpdateLayout();
		}
		else if (ContentItem->GetPackageItemType() == EPackageItemType::Component)
		{
